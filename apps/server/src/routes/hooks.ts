...
//
// 1. User opens browser, goes to /app/activate, clicks "Start activation".
//    Browser calls POST /install/device to start the flow.
//
...
//
// 3. Browser polls POST /install/poll with { device_code } every <interval> seconds.
//
// 4. User enters user_code on /app/activate.
//    Browser calls POST /install/approve (authenticated + CSRF) with { user_code }.
//    Server marks that device_code as approved and stores the hook token.
//
// 5. Next poll detects approval → returns { approved: true, hook_token }.
//
...
export async function hookRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      // Approved! Consume the device code (one-time use) and return the token.
      db.query(`DELETE FROM device_flow_sessions WHERE device_code = ?`).run(device_code);
      ...
      return reply.code(200).send({ approved: true, hook_token: token });
    },
  );
