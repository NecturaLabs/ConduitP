...
- **Persistent volume (Dokploy):** The SQLite DB lives at `/app/data/conduit.db` inside the container. Dokploy source-deploy wipes the container on every redeploy but does NOT touch Docker volumes. The volume is configured in Dokploy → service → Advanced → Volumes as a **named Docker VOLUME** (not a bind mount): Volume Name `conduit-api-data`, Mount Type `VOLUME`, Mount Path `/app/data`. Docker automatically creates and manages this named volume; it persists across all redeployments unless explicitly removed with `docker volume rm`. **Do NOT change this to a bind mount** — the host path `/var/lib/conduit/data` was the original configuration but that directory never existed on the host, which caused the container to silently fail to start on every deploy. (The `docker-compose.yml` self-hosted stack handles this via its own named Docker volume.)
